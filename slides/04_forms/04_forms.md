...
- Permitir que usuário faça consulta com base em um campo de busca onde pode indicar produtos.
...
- Neste cenário, vamos considerar:
  ...
  - Ele tem acesso a uma página (dashboard) que permite realizar buscas
    - A busca pode ser feita utilizando ***QueryString*** (String de consulta)
...
Observe-se que as requisições HTTP ilustradas no cenários e veja os caminhos (URL PATH) usados:
...
O formulário de login vai disparar uma requisição `POST /login`.
...
- `method=POST`: indicamos que o formulário será enviado via POST. Também podemos colocar GET (exemplo a seguir)
...
- O argumento `methods=['POST', 'GET']` restringe que esta rota atende apenas requisições com método HTTP **GET** e **POST**.
...
- [render_template()](https://flask.palletsprojects.com/en/3.0.x/quickstart/#rendering-templates)
- [Métodos HTTP](https://flask.palletsprojects.com/en/3.0.x/quickstart/#http-methods)
- [url_for()](https://flask.palletsprojects.com/en/3.0.x/quickstart/#url-building)
